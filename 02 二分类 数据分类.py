...
class DataProcessor(object):
    ...
    def data_classifier(cls, labels: list, contents: list) -> tuple[tuple[list, list], tuple[list, list]]:
        ...
        # 输出各个数据集的大小
        print(f"训练集数量：{red(len(train_x))}")
        print(f"验证集数量：{red(len(valid_x))}")
        print(f"数据集总数：{red(len(train_x) + len(valid_x))}")
...
def main():
    ...
    # 数据分类
    DataProcessor.data_classifier(labels, contents)
