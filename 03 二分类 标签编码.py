...
class DataEncoder(object):
    ...
    def label_encoder(cls, train: tuple[list, list], valid: tuple[list, list]) \
        ...
        valid_y = ohe.transform(valid_y).toarray()
        # 输出各个数据集的标签
        print(f"训练集标签：{train_y}")
        print(f"验证集标签：{valid_y}")
        print(f"训练集维度：{train_y.shape}")
        print(f"验证集维度：{valid_y.shape}")
...
def main():
    ...
    # 标签编码
    DataEncoder.label_encoder(train_data, valid_data)
