...
from utils import timer, green, yellow, blue, purple, lines
...
class DataEncoder(object):
    ...
    def label_encoder(cls, train: tuple[list, list], valid: tuple[list, list]) \
        ...
        # print(f"训练集标签编码维度：{yellow(train_y_ec.shape)}")
        # print(f"验证集标签编码维度：{yellow(valid_y_ec.shape)}")
        lines()
    ...
    def word_filler(cls, train: list, valid: list, max_len: int) -> tuple[list, list]:
        ...
        print(f"训练集内容填充长度：{blue(len(train_x_equal))}")
        print(f"验证集内容填充长度：{blue(len(valid_x_equal))}")
        lines()
        for line in train_x_equal[:1]:
            print(f"训练集内容填充长度：{purple(len(line))}")
        for line in valid_x_equal[:1]:
            print(f"验证集内容填充长度：{purple(len(line))}")
...
def main():
    ...
    train_contents_seq, valid_contents_seq = DataEncoder.word_filler(train_contents_seq, valid_contents_seq, max_len)
